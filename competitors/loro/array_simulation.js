...
function runBenchmarkWithSnapshots(operations) {
  ...
  // Initialize Loro document with MovableList
  const doc = new Loro();
...
async function runBenchmarks() {
  ...
  const csvRows = results.map(r => 
    `LoroArray,${r.operations},${r.timeMs},${r.opsPerSec},${r.memoryMb},${r.finalLength}`
  );
  const csvContent = [csvHeader, ...csvRows].join('\n');
  
  fs.writeFileSync(path.join(__dirname, 'loro_array_results.csv'), csvContent);
  console.log('\n✅ Results saved to loro_array_results.csv');
  console.log('🎯 Loro MovableList benchmark completed!');
